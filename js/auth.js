// js/auth.js
// A constante API_URL vem do js/api.js, que precisa ser carregado ANTES deste arquivo.
...
// NEW: keep label floated but reset color to gray (used on blur when value filled)
function floatLabelGray(label) {
  if (!label) return;
  label.style.top = "0";
  label.style.transform = "translateY(-50%)";
  label.style.fontSize = "0.75rem";
  label.style.color = ""; // remove inline color → falls back to Tailwind text-gray-1
}

// NEW: drop label back to center (used on blur when no value)
...
function bindSelectLabel(id) {
  ...
  if (!sel || !label) return;
  // FIX: float label on load if select already has value
  if (sel.value) floatLabelGray(label);
  sel.addEventListener("change", () => {
    if (sel.value) floatLabel(label);
  });
  // FIX: on blur keep float but remove red color
  sel.addEventListener("blur", () => {
    if (sel.value) floatLabelGray(label);
    else dropLabel(label);
  });
}
...
function bindDateLabel(id) {
  ...
  input.addEventListener("blur", function () {
    const label = this.closest(".relative")?.querySelector("label");
    if (this.value) floatLabelGray(label);
    else dropLabel(label);
  });
}
...
function initIndex() {
  ...
  document.getElementById("area-usuario").innerHTML = `
    <a href="./html/perfil.html" class="flex flex-col items-end leading-none cursor-pointer">
      <span class="font-inter text-red-1 font-medium text-2xl">Olá,</span>
      <span class="font-inter text-gray-1 font-bold text-2xl">${usuario.nome || "Usuário"}</span>
    </a>
  `;
}
...
// function initFaq() {
//   const usuario = JSON.parse(localStorage.getItem("usuario"));
//   const area = document.getElementById("area-usuario-faq");
//   if (!usuario || !area) return;

//   const foto = usuario.foto || "../img/foto_perfil.png";

//   area.innerHTML = `
//     <a href="./perfil.html" class="cursor-pointer">
//       <img 
//         src="${foto}" 
//         alt="foto de perfil"
//         class="w-12 h-12 rounded-full border-2 border-red-1 bg-gray-2"
//         onerror="this.src='../img/foto_perfil.png'"
//       />
//     </a>
//   `;
// }
...
function initFaq() {
  ...
  const temFoto = usuario.foto && usuario.foto.trim() !== "";
  const foto = temFoto ? usuario.foto : "../img/foto_perfil.png";
  const imgClass = temFoto
    ? "w-12 h-12 rounded-full object-cover border-2 border-red-1"
    : "w-12 h-12 rounded-full object-contain p-1.5 border-2 border-red-1 bg-gray-2";
